		...
		<div class="row">
			...
			<div class="panel-body col-sm-12" align="center">
						...
						<table align="center">
							<tr>
								...
								<td><html:text property="frmPatientName"
										styleId="frmPatientName1" styleClass="form-control"
					...
					<html:form action="/autoComplementPath">
						<logic:notEmpty property="frmPatientNameList"
							name="PatientListFormBean">
							<h4><font color="green">Number of Patient: (${PatientListFormBean.frmPatientNameList.size()})</font></h4>
							...
							<table style="border-collapse: collapse"
								...
								<c:forEach var="i" begin="${PatientListFormBean.begin}"
									...
									<tr>
										<td>${i}</td>
										<td>${PatientListFormBean.frmPatientNameList[i-1].pname}</td>
										<td>${PatientListFormBean.frmPatientNameList[i-1].pgender}</td>
										<td>${PatientListFormBean.frmPatientNameList[i-1].paddress}</td>
										<c:if
											test="${PatientListFormBean.frmPatientNameList[i-1].pstatus==0 }">
											<td>inactive</td>
											<td><a
												href="detailPath.do?frmDetailPatientId=${PatientListFormBean.frmPatientNameList[i-1].id}&frmControl=2">
													Detail</a></td>
										...
										<c:if
											test="${PatientListFormBean.frmPatientNameList[i-1].pstatus==1 }">
											<td>active</td>
											...
											<td><a
												href="detailPath.do?frmDetailPatientId=${PatientListFormBean.frmPatientNameList[i-1].id}&frmControl=2">
													Detail</a></td>
											<td><a
												href="statusChangePath.do?frmDetailStatusId=${PatientListFormBean.frmPatientNameList[i-1].id}&frmControl=3">
													Change Status</a></td>
										</c:if>
									</tr>
